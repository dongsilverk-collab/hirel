...
async function analyzeResume(jd, candidate) {
  const prompt = `당신은 HR 전문가 AI입니다. 채용 공고와 후보자 이력서를 분석하여 적합도를 평가하세요.
...
순수 JSON으로만:
{"totalScore":숫자,"scores":{"experienceMatch":숫자,"cultureFit":숫자,"skillKeywords":숫자,"stability":숫자},"verdict":"추천"|"검토필요"|"부적합","strengths":["강점1","강점2","강점3"],"weaknesses":["약점1","약점2"],"keywords":[{"word":"키워드","type":"positive"|"negative"|"neutral"}],"interviewQuestions":["질문1","질문2","질문3"],"summary":"2-3문장 요약"}`;
...
async function scoreInterview(jd, name, transcript, questions) {
  const prompt = `당신은 면접관 AI입니다. 면접 녹취록을 실시간 평가하세요.
채용 공고: ${jd.slice(0, 400)}
후보자: ${name}  질문 목록: ${questions?.join(" / ") || "자유 면접"}
=== 녹취록 ===
${transcript}
순수 JSON으로만:
{"liveScore":숫자,"dimensions":{"communication":숫자,"expertise":숫자,"motivation":숫자,"problemSolving":숫자,"culture":숫자},"highlights":["포인트1","포인트2"],"concerns":["우려1"],"nextQuestion":"추천 다음 질문","oneliner":"한줄 평가"}`;
...
function InterviewRoom({ candidate, position, onBack }) {
  ...
  const start = () => {
    ...
    r.onresult = (e) => { let fin = "", int = ""; for (let i = e.resultIndex; i < e.results.length; i++) { if (e.results[i].isFinal) fin += e.results[i][0].transcript + " "; else int += e.results[i][0].transcript; } if (fin) setTranscript(p => { const n = p + fin; txRef.current = n; return n; }); setInterim(int); };
    r.onend = () => { if (recogRef.current === r) r.start(); };
    r.start(); recogRef.current = r; setRecording(true);
    timerRef.current = setInterval(() => setElapsed(p => p + 1), 1000);
    if (auto) autoRef.current = setInterval(() => { if (txRef.current.trim().length > 30) doScore(txRef.current); }, 25000);
  };
  const stop = () => { recogRef.current?.stop(); recogRef.current = null; clearInterval(timerRef.current); clearInterval(autoRef.current); setRecording(false); setInterim(""); };
  const doScore = async (tx) => {
    if (!tx || tx.trim().length < 10 || scoring) return;
    setScoring(true);
    try { const r = await scoreInterview(position?.jd || "", candidate.name, tx, candidate.analysis?.interviewQuestions); setLiveScore(r); setHistory(p => [...p.slice(-19), { score: r.liveScore }]); }
    catch (e) { console.error(e); }
    setScoring(false);
  };
  ...
  useEffect(() => () => { recogRef.current?.stop(); clearInterval(timerRef.current); clearInterval(autoRef.current); }, []);
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 22 }}>
        ...
        <div>
          ...
          <span style={{ fontSize: 12, color: C.sub }}>{recording ? "🔴 녹음 중" : "대기 중"}</span>
        </div>
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 7, background: C.card, border: `1px solid ${C.border}`, borderRadius: 9, padding: "7px 13px" }}>
            <span style={{ fontSize: 12, color: C.sub }}>자동평점</span>
            <div onClick={() => setAuto(p => !p)} style={{ width: 34, height: 18, borderRadius: 9, background: auto ? C.accent : C.border, cursor: "pointer", position: "relative", transition: "all .2s" }}>
              <div style={{ position: "absolute", top: 2, left: auto ? 18 : 2, width: 14, height: 14, borderRadius: "50%", background: "#fff", transition: "all .2s" }} />
            </div>
          </div>
          {!recording
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 370px", gap: 18 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {candidate.analysis?.interviewQuestions && (
            <div style={{ background: C.card, borderRadius: 13, border: `1px solid ${C.border}`, padding: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: C.sub, marginBottom: 11 }}>💬 면접 질문</div>
                    ...
                    {activeTab === "interview" && (
                      <div style={{ background: C.card, borderRadius: 13, border: `1px solid ${C.border}`, padding: 24 }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
                          <div><h3 style={{ fontSize: 13, fontWeight: 700, marginBottom: 3 }}>AI 추천 면접 질문</h3><p style={{ fontSize: 12, color: C.sub, margin: 0 }}>약점 기반 맞춤 생성</p></div>
