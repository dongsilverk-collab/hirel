...
const KIT_QUESTIONS = {
  ...
  "김소영": [
    ...
    { text: "뉴트리엄에서 3년 8개월 — 캐나다 건기식 회사에서 정확히 어떤 업무를 하셨나요? 하루 일과를 걸어가 주세요.", pass: "업무 범위가 구체적(콘텐츠·광고·유통 중 무엇인지 분리됨)", red: "역할이 모호 / 마케팅이 아니라 운영·CS 중심이었음이 드러남" },
    { text: "캐나다 시장과 한국 시장 — 매체가 다릅니다. 네이버·카카오·쿠팡을 직접 운영해본 적 있나요? 없다면 첫 달에 어떻게 따라잡으시겠어요?", pass: "갭을 인정하고 학습 계획이 구체적 (Meta 글로벌 경험은 이전 가능)", red: "'금방 배운다'로 뭉갬 / 국내 매체 이해가 0에 가까움" },
    { text: "캐나다는 NHP(자연건강제품) 규제, 한국은 건기식 심의 — 표현 규제 차이를 체감한 사례가 있나요? '면역력 강화' 같은 표현, 한국에서 일반식품이면 어떻게 됩니까?", pass: "규제 차이 인지 + 한국 심의 감각의 단서", red: "규제를 의식해본 적 없음" },
    { text: "저희 공고를 여러 번 스크랩하셨더라고요 — 큐라엘의 뭘 보셨나요? 유튜브나 자사몰 보셨으면 솔직히 아쉬운 점 하나만요.", pass: "실제로 본 흔적 + 비판 섞인 솔직함(동기의 실체)", red: "칭찬만 / 사실 잘 모름이 드러남" },
    { text: "지금까지 만든 소재나 콘텐츠 중 성과가 제일 좋았던 것 하나 — 기획부터 제작까지 본인 손이 어디까지였나요?", pass: "직접 제작의 실물 + 기여 분리", red: "팀·외주 결과물을 본인 것처럼" },
    { text: "귀국 후 커리어를 한국에서 이어가시는 건데, 3년 뒤 어떤 모습이면 '이 선택이 맞았다'고 하시겠어요?", pass: "우리 구조에서 실현 가능한 그림 + 한국 정착 의지", red: "모호 / 해외 복귀 가능성이 비침" },
  ],
};
...
const CAREER_BUILTIN = {
  ...
  "이시라": {
    ...
    note: "희망 5,500(예산 정합) · 2주 내 입사 가능 · 강동구 길동. ⚠ 그룹바이 1군이었던 방주환과 같은 회사(블렌디드) 출신 — 조직·본인 평판 교차 검증 기회",
  },
  "김남수": {
    ...
    note: "⚠ 현재 8,500/희망 9,500 — 총괄 예산 결정 필요(최초반 선고지). AI: Claude Code·Codex 자동화 파이프라인 직접 운영. 지원서에서 큐라엘 모델('제품·정보·상담 연결')을 정확히 명시한 유일한 후보",
  },
  "황유정": {
    ...
    note: "현 4,999(고정급)/희망 협의 — 예산 정합 ✅ · 구로구 신도림 · JnCHR 조천수 20%. ⚠ 매체 볼륨(월 광고비 규모) 미기재 — 면접에서 확인",
  },
  "김소영": {
    birth: 1994, edu: "상세 학력·자격 미확보 (잡코리아 지원서 — 이력서 PDF 미다운로드)",
    header: "총 3.8년 — 뉴트리엄(캐나다 건기식) · ⚠ 상세 경력은 잡코리아 지원자 페이지에서 면접 전 확보 필요",
    rows: [
      { company: "뉴트리엄 (캐나다)", role: "건기식 마케팅 (추정)", period: "~최근", dur: "3년 8개월", work: "캐나다 건강기능식품 기업 — 업무 범위(콘텐츠/광고/유통) 미상, 면접에서 하루 일과 워크스루로 확인" },
    ],
    note: "잡코리아 '건기식 콘텐츠 마케터' 공고 지원자(08.24 스크리닝 s01, 1순위 A-) — 연봉 '면접 후 결정'(미기재, 최초반 선고지 필수). ✅ 우리 공고를 반복 스크랩 = 관심 지속. 리스크: 해외 경력의 국내 매체(네이버·카카오·쿠팡) 적응",
  },
  "왕태일": {
...
const FOCUS_BUILTIN = {
  ...
  "김소영": {
    verdict: "판정 기준: ①국내 매체 적응 가능성 + ②뉴트리엄 업무 실체 확인 — 통과 시 실행 트랙 후보 (⚠ 이력서 상세 미확보 상태로 면접 진행 주의)",
    points: [
      ["🔑 업무 실체", "'캐나다 건기식 3.8y'가 전부 — 하루 일과 워크스루로 콘텐츠/광고/유통/CS 중 실제 역할을 확인. 면접 전 잡코리아에서 이력서 PDF 확보 권장"],
      ["🔑 국내 매체 갭", "네이버·카카오·쿠팡 직접 운영 경험 여부 — 없으면 학습 계획의 구체성으로 판단 (Meta 글로벌 경험은 이전 가능)"],
      ["연봉 미기재", "'면접 후 결정' — 최초반 선고지(상한 5,500)로 숫자를 먼저 받아라"],
      ["✅ 동기 강점", "우리 공고 반복 스크랩 — '큐라엘의 뭘 봤는지 + 아쉬운 점 하나' 질문으로 동기의 실체와 솔직함을 동시 확인"],
    ],
  },
};
